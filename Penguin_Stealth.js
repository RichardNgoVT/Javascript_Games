...
//A/D: Turn left/right
//Spacebar: Slide, restart game when game over or game win occurs
...
//your way around the 400 by 400 pixel map in order to succeed. In order to win, you must be able to get all 20 fishes in the map. 
//The polar bear's line of sight are indicated by the yellow archs in front of them. If you are caught within the yellow arch, you will be chased. 
//However, if there is a wall between you and the bear, you will not be spotted even if you do get within the yellow arch. When not chasing you, 
//the polar bears will wander. If you are able to lose the bears while being chased,the polar bear will go to the last place that it saw you before 
//resuming to wander. The polar bears will out run your default movement speed, but you are faster then the polar bears while you are sliding. 
//However, the downsides of sliding is that you can only move forwards, can not turn, and that the slide has a start up time before it can be used, 
//so you can not slide around too carelessly.If you are caught before you get to all 20 fish, you will reach the game over screen. 
//If you are able to get all 20 fishes before being caught, you will reach the success screen, and win the game.
...
bearObj.prototype.act = function() {
   ...
   var see = true;
   //if within range
        if(mag(this.pos.x-penguin[0].pos.x,this.pos.y-penguin[0].pos.y)<90){
            //println("inrange");
            //bear is facing right direction
            if(atan2(penguin[0].pos.x-this.pos.x,penguin[0].pos.y-this.pos.y)<this.angle+PI/2-PI/5 && atan2(penguin[0].pos.x-this.pos.x,penguin[0].pos.y-this.pos.y)>this.angle-PI/2+PI/5){
              //  println("right direction");
                //check for walls
            for(var i = 0; i <walls.length;i++){
            if(mag(walls[i].x-this.pos.x,walls[i].y-this.pos.y)+mag(penguin[0].pos.x-walls[i].x,penguin[0].pos.y-walls[i].y) < mag(this.pos.x-penguin[0].pos.x,this.pos.y-penguin[0].pos.y)+10){
        see = false;//if wall, can't see
        break;
            }
            }
            if(see){//if can see, update target location and set chase to true
            this.travel.x = penguin[0].pos.x;
            this.travel.y = penguin[0].pos.y;
			this.chase = true;
			//println("found");
            }
            }
